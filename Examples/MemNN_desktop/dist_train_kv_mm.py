...
def partition_dataset():
    ...
    return train_q[start_index:end_index], train_w[start_index:end_index], train_e_p[start_index:end_index], train_a[start_index:end_index],
...
def run(rank, size):
   ...
   model.load_embed('./model/300.org/embedding.pre_0')
   ...
   print "{} batch expected".format(len(dataset[0]) * config.epoch / config.batch_size)
   ...
   for e_ in range(config.epoch):
       ...
       for i_q, i_k, i_v, i_cand, i_a in zip(train_q, train_key, train_value, train_cand, train_a):
           ...
           if cnt % config.valid_every == 0:
               if rank == 0:
                   print "Accuracy:", eval(model, dev_q, dev_key, dev_value, dev_cand, dev_a)
